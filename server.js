...
app.get('/api/station-notes/for-stage/:stageIndex', requireStationUser, async (req, res) => {
  try {
    ...
    if (!Number.isInteger(stage) || stage <= 0) return res.json([]); // stage 0 has nothing upstream
    // Forward-broadcast: an operator at this stage sees notes from EVERY
    // upstream stage on the jobs currently at their station. CTP's message
    // reaches Printing, Coating, Die-Cut, Break, Paste, Storage, and
    // Delivered — so a heads-up doesn't get lost when the job moves past
    // the next station.
    const rows = await sql`
